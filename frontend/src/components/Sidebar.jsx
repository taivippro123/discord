...
const Sidebar = ({ user, setActiveServer }) => {
  ...
  const handleCreateServer = async () => {
    ...
    try {
      const res = await axiosInstance.post(`${API_BASE_URL}/create-server`, {
        name: newServerName,
        user_id: user.id,
      });
  ...
  const handleJoinServer = async () => {
    ...
    try {
      const joinRes = await axiosInstance.post(`${API_BASE_URL}/join-server`, {
        invite_code: inviteCode,
        user_id: user.id,
      });

      const updatedServersRes = await axiosInstance.get(
        `${API_BASE_URL}/servers/${user.id}`
      );
      setServers(updatedServersRes.data);

      // Chọn server mới tham gia
      const newServerId = joinRes.data.server_id;
      const joinedServer = updatedServersRes.data.find(
        (s) => s.id === newServerId
      );
      if (joinedServer) {
  ...
  return (
    <div className="w-[72px] h-full bg-[#1e1f22] flex flex-col overflow-hidden">
      {/* Nút tạo server - cố định trên cùng */}
        ...
        <div className="flex flex-col items-center w-full mt-6">
          {servers.map((server) => (
            ...
            >
              {/* Server indicator */}
              <div
                className={`absolute -left-2 w-1.5 h-8 rounded-r-full transition-all ${
                  activeServerId === server.id
                    ? "bg-white"
